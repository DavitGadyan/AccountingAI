...
export const ICONS: Record<IconKey, Draw> = {
  // A key: authentication. Distinct from `shield`, which is authorisation — the two
  // sit next to each other in the access stage and must not read as the same node.
  key: stroke((ctx, s) => {
    ctx.beginPath();
    ctx.arc(s * 0.32, s * 0.4, s * 0.16, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(s * 0.66, s * 0.7);
    ctx.lineTo(s * 0.56, s * 0.8);
    ctx.stroke();
  }),

  // A bound ledger: the fact base — documents turned into a reviewed record. Reads as
  // "the file" rather than as "a database", which is the correct mental model here.
  ledgerbook: stroke((ctx, s) => {
    ctx.beginPath();
    ctx.roundRect(s * 0.18, s * 0.16, s * 0.64, s * 0.68, s * 0.07);
    ctx.stroke();
    ctx.beginPath();
