 ...
 *
 * Tightened further because `zoomToFit` measures the *3D* bounding box: height and
 * depth are unpinned, so the box is far taller and deeper than the flat ribbon the
 * camera actually sees, and fitting it left the pipeline occupying a third of the
 * frame with dead space all around.
 */
const FIT_TIGHTEN = 0.58;
 ...
 *
 * Raised from 0.006: at a 1600×1000 viewport a stage disc came out around 54px, which
 * renders the glyph inside it at roughly 30px — enough to see a shape, not enough to
 * read which shape. The glyph is the only thing distinguishing two same-coloured nodes
 * at a glance, so it has to survive being looked at on a recording.
 */
const NODE_SCREEN_SCALE = 0.0085;
...
export function Graph3D({
  ...
  //
  // The renderer defaults to a device pixel ratio of 1, so on any retina screen the
  // whole scene is drawn at half resolution and scaled up — every edge and glyph
  // arrives soft no matter how sharp the source texture is. Capped at 2 because the
  // gain above that is invisible and the fill cost is quadratic.
  React.useEffect(() => {
  ...
  const configureForces = React.useCallback(() => {
    ...
    // Raised from -1800: the isolated view of a ten-child card is the densest thing
    // the graph ever draws, and at the old strength its labels settled on top of one
    // another. Repulsion is what separates them once the orbit is already wide.
    charge.strength(-3200).distanceMax(1800);
    ...
    fgRef.current
      ...
      ?.distance((link: GraphLink) => {
        if (link.kind !== "tree") return FLOW_SPACING * 1.5;

        // Scale the orbit with how many siblings share it.
        //
        // A fixed radius is really a fixed *circumference*, so it silently becomes a
        // per-child allowance that shrinks as children are added. At three children the
        // labels have room; at the ten inside the determination engine they collide into
        // an unreadable knot — which is worst on the one card the whole demo opens.
        const parentId =
          typeof link.source === "object"
            ? (link.source as GraphNode).id
            : (link.source as string);
        const siblings = CHILDREN_BY_PARENT.get(parentId)?.length ?? 1;
        // Linear rather than sqrt: children spread in three dimensions but the labels
        // are read in two, so a chunk of any orbit is lost to foreshortening and a
        // square-root allowance does not keep up past about six children.
        return CHILD_DISTANCE * Math.max(1, siblings / 4.5);
      });
  ...
  return (
    ...
    >
      <ForceGraph3D<GraphNode, GraphLink>
        ...
        linkColor={linkColor}
        // Link width is in WORLD units, and the pipeline spans roughly 2,500 of them
        // (FLOW_SPACING x 7). The stock 0.6–1.6 therefore renders sub-pixel at any
        // framing that fits the whole diagram: the edges came out as faint aliased
        // threads that read as blur rather than as connections. These values put a
        // request edge at a few solid pixels when the whole graph is on screen.
        linkWidth={(link) =>
          link.kind === "request"
            ? 9
            : link.kind === "improve"
        ...
        // Raised from 0.5. Combined with sub-pixel widths, half-opacity edges
        // disappeared entirely against the light background.
        linkOpacity={0.78}
        linkResolution={12}
        linkDirectionalParticles={(link) =>
          link.kind === "request" ? 3 : link.kind === "improve" ? 2 : 0
        }
...
/**
 * Canvas sprites are drawn once at high resolution and then minified on screen.
 *
 * Three's default `minFilter` for a CanvasTexture samples a generated mipmap, so the GPU
 * reads a pre-blurred half-size copy of the text rather than the texture that was
 * actually drawn — which is what makes labels and glyphs look soft however much the
 * canvas is supersampled. Turning mipmaps off costs nothing here (these sprites are
 * screen-space sized, so they never recede far enough to alias) and is the single
 * difference between crisp and fuzzy.
 *
 * `colorSpace` matters for the same reason contrast matters: without it the canvas is
 * treated as linear and every label renders washed out against its halo.
 */
function sharpen(texture: THREE.CanvasTexture): void {
